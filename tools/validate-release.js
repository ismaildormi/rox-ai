...
function checkJson() {
  ...
  for (const file of files) {
    try {
      ...
      fail(`JSON parse: ${path.relative(root, file)} — ${error.message}`);
      return;
    }
  }
...
function checkEmbeddedScripts(documents) {
  for (const [name, html] of Object.entries(documents)) {
    ...
    try {
      ...
      fail(`${name} embedded JavaScript — ${error.message}`);
    }
  }
}
...
function checkI18n(wrapper) {
  ...
  pass(`i18n synchronization (${firstKeys.length} keys × 5 languages × 2 copies)`);
}
...
function main() {
  ...
  try {
    ...
    fail(`Embedded document extraction — ${error.message}`);
  }
